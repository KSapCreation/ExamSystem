...
Public Class frmCreateEXE
    Inherits FrmMainTranScreen
   
    Dim frmHeading As String = ""
    Public isNewEntry As Boolean = False
    Const colchkTicketNo As String = "Ticket No"
    ...
    End Sub
    Sub LoadType()
        cboType.DataSource = clsCreateExe.LoadType()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Version_No) > 0) Then
            ...
            txtVersion.Text = obj.Version_No
    ...
    Private Function AllowToSave() As Boolean
        Try
            If clsCommon.myLen(clsCommon.myCstr(txtClient.Value)) <= 0 Then
                clsCommon.MyMessageBoxShow("Client Code cant not be blank.", Me.Text)
                txtClient.Focus()
                txtClient.Select()
                Return False
            End If
            If clsCommon.myLen(clsCommon.myCstr(txtVersion.Text)) <= 0 Then
                clsCommon.MyMessageBoxShow("Version can not be blank.", Me.Text)
                txtVersion.Focus()
                txtVersion.Select()
                Return False
            End If
            If clsCommon.myLen(clsCommon.myCstr(cboType.SelectedValue)) <= 0 Then
                clsCommon.MyMessageBoxShow("Version can not be blank.", Me.Text)
                cboType.Focus()
                cboType.Select()
                Return False
    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        Try
            ...
            Dim BackVersion As String = clsDBFuncationality.getSingleValue("select top 1 Version from TSPL_VERSION_HEAD where client_code='" & txtClient.Value & "' and type='E' order by Version_date desc")
            ...
            If clsCommon.myLen(BackVersion) > 0 Then
                qry += "  and TSPL_TIME_SHEET_DETAIL.EXE_VERSION>='" & BackVersion & "' and TSPL_TIME_SHEET_DETAIL.EXE_VERSION<='" & txtVersion.Text & "'"
            End If
    ...
    Sub LoadBlankGrid()
        ...
        chkTicket_No.Name = colchkTicketNo
    ...
    Private Sub txtClient__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtClient._MYValidating
        ...
        txtClient.Value = clsCommon.ShowSelectForm("Clit@Type", qry, "Code", WhrCls, txtClient.Value, "Code", isButtonClicked)
    ...
    Sub SaveData()
        Try
            If AllowToSave() Then
                ...
                obj.Version_No = txtVersion.Text
    ...
    Private Sub btnUnselect_Click_1(sender As Object, e As EventArgs) Handles btnUnselect.Click
        UnselectAll()
    End Sub
